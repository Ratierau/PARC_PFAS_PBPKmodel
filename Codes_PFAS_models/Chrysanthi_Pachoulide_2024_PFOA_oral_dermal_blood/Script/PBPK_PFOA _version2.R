...
Dermconc = as.numeric(SumExpPFOA_LB_val[i,14])
...
PBPKmodPFOA <- function(t,state,parameters){
  with(as.list(c(state,parameters)), {
    ...
    #If the time (t) since the last input (Tinput) is less than the dosing interval (tinterval) then calculate the input rate of the dose, if not then input should be 0
    ...
    #                  to liver                                                           #assumption that mesenteric and portal flow are equal (QGut = QPortal)
